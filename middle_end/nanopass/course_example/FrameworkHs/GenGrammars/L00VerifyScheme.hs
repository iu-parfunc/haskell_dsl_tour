...
module FrameworkHs.GenGrammars.L00VerifyScheme where
...
instance PP Expr where
  ...
  pp (Letrec l l2) = (ppSexp (fromByteString "letrec" : ((ppSexp (map (\(u,e) -> (ppSexp [(pp u),(pp e)])) l)) : (map pp l2))))
  pp (Lambda l l2) = (ppSexp (fromByteString "lambda" : ((ppSexp (map pp l)) : (map pp l2))))
  pp (And l) = (ppSexp (fromByteString "and" : (map pp l)))
  pp (Or l) = (ppSexp (fromByteString "or" : (map pp l)))
  pp (Not e) = (ppSexp [fromByteString "not",(pp e)])
  pp (If1 e e2) = (ppSexp [fromByteString "if",(pp e),(pp e2)])
  pp (If2 e e2 e3) = (ppSexp [fromByteString "if",(pp e),(pp e2),(pp e3)])
  ...
  ppp (Letrec l l2) = (pppSexp (text "letrec" : ((pppSexp (map (\(u,e) -> (pppSexp [(ppp u),(ppp e)])) l)) : (map ppp l2))))
  ppp (Lambda l l2) = (pppSexp (text "lambda" : ((pppSexp (map ppp l)) : (map ppp l2))))
  ppp (And l) = (pppSexp (text "and" : (map ppp l)))
  ppp (Or l) = (pppSexp (text "or" : (map ppp l)))
  ppp (Not e) = (pppSexp [text "not",(ppp e)])
  ppp (If1 e e2) = (pppSexp [text "if",(ppp e),(ppp e2)])
  ppp (If2 e e2 e3) = (pppSexp [text "if",(ppp e),(ppp e2),(ppp e3)])
